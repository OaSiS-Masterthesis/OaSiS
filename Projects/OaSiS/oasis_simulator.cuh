...
struct OasisSimulator {
	...
	std::vector<surface_flow_model_t> surface_flow_models = {};
	std::vector<std::array<int, 2>> surface_flows = {};
	...
	std::vector<float> surface_flow_mass_transfer_host_buffer = {};
	...
	void init_solid_fluid_coupling(const int solid_id, const int fluid_id) {
		if(solid_id < particle_counts.size() && fluid_id < particle_counts.size()){
			...
			if(!already_coupled){
				...
				iq_fluid_particle_buffers.emplace_back(iq::FluidParticleBuffer(managed_memory_allocator, &managed_memory, particle_counts[fluid_id] / config::G_BIN_CAPACITY + config::G_MAX_ACTIVE_BLOCK));
			}
		}else{
	...
	void init_surface_flow(const int surface_id, const int flow_id) {
		if(surface_id < particle_counts.size() && flow_id < surface_flow_models.size()){
			bool already_coupled = false;
			for(size_t i = 0; i < surface_flows.size(); ++i){
				if(surface_flows[i][0] == surface_id || surface_flows[i][1] == surface_id){
					already_coupled = true;
					fmt::print("Model already has surface_flow: {}<->{} but already exists {}<->{}", surface_id, flow_id, surface_flows[i][0], surface_flows[i][1]);
					break;
				}
			}
			
			if(!already_coupled){
				surface_flows.push_back({surface_id, flow_id});
			...
			fmt::print("Model ids out of range: {} {} but model count is {} and surface flow model count is {}", surface_id, flow_id, particle_counts.size(), surface_flow_models.size());
		}
	}
	...
	void update_surface_flow_parameters() {
	}
			...
			//for(Duration current_step_time = Duration::zero(); current_step_time < Duration::zero(); current_step_time += dt, cur_time += dt, cur_step++) {
				...
				//IQ-Solve!
				{
					...
					*/
					for(int i = 0; i < solid_fluid_couplings.size(); ++i){
						...
						iq_result_array.resize_and_reset(lhs_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME);
						
						iq_lhs_rows.resize_and_reset(lhs_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME + 1);
						
						iq_solve_velocity_result_array.resize_and_reset(3 * solve_velocity_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME);
						
						iq_solve_velocity_rows.resize_and_reset(3 * solve_velocity_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME + 1);
						...
						if(has_surface_flow){
							...
							*/
							match(particle_bins[rollid][solid_id], particle_bins[rollid][fluid_id])([this, &i, &cu_dev, &coupling_block_count, &solid_id, &fluid_id](auto& particle_buffer_solid, auto& particle_buffer_fluid) {
								...
								managed_memory.acquire<MemoryType::DEVICE>(
									...
									, iq_fluid_particle_buffers[i].acquire()
						...
						for(size_t temporary_block = 0; temporary_block < solve_velocity_matrix_total_block_count; ++temporary_block){
						...
						for(size_t temporary_block = 0; temporary_block < lhs_matrix_size_y; ++temporary_block){
						...
						if(has_surface_flow){
							...
							*/
							...
							//FIXME: Use b! identity->apply(gko_neg_one_dense, b, gko_one_dense, iq_rhs_parts[2]);
							...
							temporary_matrices[0]->apply(gko_neg_one_dense, velocity_fluid, gko_one_dense, iq_rhs_parts[3]);
						...
						const std::shared_ptr<const gko::matrix::Csr<float, int>> iq_solve_velocity = gko::share(
							gko::matrix::Csr<float, int>::create_const(
								  ginkgo_executor
								, gko::dim<2>(3 * solve_velocity_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME, solve_velocity_matrix_size_x * exterior_block_count * config::G_BLOCKVOLUME)
						...
						std::shared_ptr<gko::matrix::Dense<float>> iq_solve_velocity_result = gko::share(
							gko::matrix::Dense<float>::create(
								  ginkgo_executor
								, gko::dim<2>(3 * solve_velocity_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME, 1)
						...
						*/
						...
						cudaMemcpyAsync(printout_tmp0.data(), iq_solve_velocity->get_const_row_ptrs(), sizeof(int) * (3 * solve_velocity_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME + 1), cudaMemcpyDefault, cu_dev.stream_compute());
						...
						//std::cout << std::endl;
						//for(size_t j = 0; j < 3 * solve_velocity_matrix_size_y * exterior_block_count * config::G_BLOCKVOLUME + 1; ++j){
						//	std::cout << printout_tmp0[j] << " ";
						//}
						//std::cout << std::endl;
						...
						for(size_t j = 0; j < solve_velocity_matrix_size_y; ++j){
							for(size_t k = 0; k < exterior_block_count * config::G_BLOCKVOLUME; ++k){
						...
						*/
						...
						std::cout << std::endl;
						for(size_t k = 0; k < solve_velocity_matrix_size_y; ++k){
						...
						const std::array<size_t, lhs_matrix_size_y> tmp_num_blocks_per_row = {
							  2
							, 3
							, 3
							, 4
						...
						for(size_t k = 0; k < lhs_matrix_size_y; ++k){
							for(size_t j = 0; j < exterior_block_count * config::G_BLOCKVOLUME; ++j){
								...
								if(printout_tmp5[k * exterior_block_count * config::G_BLOCKVOLUME + j] != 0.0f){
									...
									std::cout << "A: " << std::endl;
									for(size_t k1 = 0; k1 < lhs_matrix_size_y; ++k1){
				...
				/// g2p2g
				{
					...
					for(int i = 0; i < surface_flows.size(); ++i) {
						if(checked_bin_counts[surface_flows[i][0]] > 0) {
							surface_flow_particle_buffers[(rollid + 1) % BIN_COUNT][i].resize(managed_memory_allocator, checked_bin_counts[surface_flows[i][0]]);
						}
					}
					
					for(size_t i = 0; i < solid_fluid_couplings.size(); ++i){
						if(checked_bin_counts[solid_fluid_couplings[i][1]] > 0) {
							iq_fluid_particle_buffers[i].resize(managed_memory_allocator, checked_bin_counts[solid_fluid_couplings[i][1]]);
						}
					}
					...
					for(int i = 0; i < get_model_count(); ++i) {
						...
						bool is_coupled_as_fluid = false;
						int coupling_index = -1;
						for(size_t j = 0; j < solid_fluid_couplings.size(); ++j){
							if(solid_fluid_couplings[j][1] == i){
								is_coupled_as_fluid = true;
								is_coupled = true;
								coupling_index = j;
								break;
							}else if(solid_fluid_couplings[j][0] == i){
								is_coupled = true;
								coupling_index = j;
								break;
							}
				...
				//TODO: Volume redistribute on shell, volume exchange between shell and domain, volume exchange between triangle mesh and domain (maybe already done in simulation step and inner shell update step?!)
				{
					...
					for(int i = 0; i < get_model_count(); ++i) {
						bool is_coupled = false;
						for(size_t j = 0; j < solid_fluid_couplings.size(); ++j){
							if(solid_fluid_couplings[j][0] == i || solid_fluid_couplings[j][1] == i){
								is_coupled = true;
								break;
							}
						}
						...
						for(int j = 0; j < triangle_shells[(rollid + 1) % BIN_COUNT][i].size(); ++j) {
							...
							match(particle_bins[rollid][i])([this, &cu_dev, &i, &j, &is_coupled](auto& particle_buffer) {
								auto& next_particle_buffer = get<typename std::decay_t<decltype(particle_buffer)>>(particle_bins[(rollid + 1) % BIN_COUNT][i]);
								...
								//partition_block_count; G_PARTICLE_BATCH_CAPACITY
								cu_dev.compute_launch({partition_block_count, config::G_PARTICLE_BATCH_CAPACITY}, shell_to_grid, dt, next_dt, is_coupled, partition_block_count, particle_buffer, get<typename std::decay_t<decltype(particle_buffer)>>(particle_bins[(rollid + 1) % BIN_COUNT][i]), triangle_meshes[i], triangle_shells[rollid][i][j], triangle_shells[(rollid + 1) % BIN_COUNT][i][j], triangle_shells[(rollid + 1) % BIN_COUNT][i][j].particle_buffer, partitions[rollid], grid_blocks[1][i]);
	...
	void output_model() {
		...
		for(int i = 0; i < get_model_count(); ++i) {
			...
			match(particle_bins[(rollid + 1) % BIN_COUNT][i])([this, &cu_dev, &surface_flow_id, &particle_count, &i, &particle_id_mapping_buffer, &surface_vertex_count_ptr, &surface_triangle_count_ptr](auto& particle_buffer) {
				...
				std::cout << "Thrust call 2 Start. Count: " << partition_block_count << std::endl;
				auto policy = thrust::cuda::par.on(static_cast<cudaStream_t>(cu_dev.stream_compute()));
				...
				std::cout << "Thrust call 2 End." << std::endl;
			});
			...
			void* output_transfer_device_buffer_ptr = surface_transfer_device_buffer;
			
			managed_memory.acquire<MemoryType::DEVICE>(
				  particles[i].acquire()
				, reinterpret_cast<void**>(&output_transfer_device_buffer_ptr)
			);
			...
			managed_memory.acquire<MemoryType::DEVICE>(
				&output_transfer_device_buffer_ptr
			);*/
			...
			
			{
				...
				//Copy the data to the output model
				check_cuda_errors(cudaMemcpyAsync(surface_point_type_transfer_host_buffer.data(), output_transfer_device_buffer_ptr, sizeof(int) * (particle_count), cudaMemcpyDefault, cu_dev.stream_compute()));
			}
			
			{
				...
				//Copy the data to the output model
				check_cuda_errors(cudaMemcpyAsync(surface_normal_transfer_host_buffer.data(), output_transfer_device_buffer_ptr, sizeof(std::array<float, config::NUM_DIMENSIONS>) * (particle_count), cudaMemcpyDefault, cu_dev.stream_compute()));
			}
			
			{
				...
				//Copy the data to the output model
				check_cuda_errors(cudaMemcpyAsync(surface_mean_curvature_transfer_host_buffer.data(), output_transfer_device_buffer_ptr, sizeof(float) * (particle_count), cudaMemcpyDefault, cu_dev.stream_compute()));
			}
			...
			if(surface_flow_id != -1){
				match(particle_bins[rollid][i])([this, &cu_dev, &surface_flow_id, &i, &d_particle_count, &particle_id_mapping_buffer, &output_transfer_device_buffer_ptr](const auto& particle_buffer) {
					//partition_block_count; G_PARTICLE_BATCH_CAPACITY
					cu_dev.compute_launch({partition_block_count, config::G_PARTICLE_BATCH_CAPACITY}, retrieve_particle_buffer_surface_flow, partitions[rollid], partitions[(rollid + 1) % BIN_COUNT], particle_buffer, get<typename std::decay_t<decltype(particle_buffer)>>(particle_bins[(rollid + 1) % BIN_COUNT][i]), surface_flow_particle_buffers[rollid][surface_flow_id], particle_id_mapping_buffer, static_cast<float*>(output_transfer_device_buffer_ptr));
				});
				
				cu_dev.syncStream<streamIdx::COMPUTE>();
				
				//Copy the data to the output model
				check_cuda_errors(cudaMemcpyAsync(surface_flow_mass_transfer_host_buffer.data(), output_transfer_device_buffer_ptr, sizeof(float) * (particle_count), cudaMemcpyDefault, cu_dev.stream_compute()));
			}
			
			match(particle_bins[(rollid + 1) % BIN_COUNT][i])([this, &cu_dev, &surface_flow_id, &particle_count, &i, &particle_id_mapping_buffer, &output_transfer_device_buffer_ptr](auto& particle_buffer) {
				auto& prev_particle_buffer = get<typename std::decay_t<decltype(particle_buffer)>>(particle_bins[rollid][i]);
	...
	void initial_setup() {
		...
		
		{
			...
			for(size_t i = 0; i < solid_fluid_couplings.size(); ++i){
				match(particle_bins[rollid][solid_fluid_couplings[i][1]])([this, &cu_dev, &i](auto& particle_buffer) {
					particle_buffer.particle_bucket_sizes = particle_buffer.particle_bucket_sizes_virtual;
					particle_buffer.bin_offsets = particle_buffer.bin_offsets_virtual;
					managed_memory.acquire<MemoryType::DEVICE>(
						  particle_buffer.acquire()
					...
					//Initialize iq_fluid_particle_buffers
					//partition_block_count; G_PARTICLE_BATCH_CAPACITY
					cu_dev.compute_launch({partition_block_count, config::G_PARTICLE_BATCH_CAPACITY}, iq::init_fluid_particle_buffer, particle_buffer, iq_fluid_particle_buffers[i], vel0[solid_fluid_couplings[i][1]].data_arr());
