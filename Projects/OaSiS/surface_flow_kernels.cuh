...
constexpr size_t SIMPLE_SURFACE_FLOW_LHS_MATRIX_TOTAL_BLOCK_COUNT = 20;
...
constexpr size_t SIMPLE_SURFACE_FLOW_SOLVE_VELOCITY_MATRIX_SIZE_X = 5;

constexpr size_t SIMPLE_SURFACE_FLOW_SOLVE_VELOCITY_MATRIX_TOTAL_BLOCK_COUNT = 8;
...
, float* __restrict__ scaling_solid, float* __restrict__ pressure_solid_nominator, float* __restrict__ pressure_solid_denominator, float* __restrict__ mass_solid, float* __restrict__ gradient_solid, float* __restrict__ coupling_solid
...
, float* __restrict__ coupling_domain, float* __restrict__ surface_flow_coupling_domain
	...
	if(has_neighbours){
		...
		for(size_t local_cell_index = 0; local_cell_index < iq::get_thread_count<iq::BLOCK_SIZE, (3 * iq::NUM_COLUMNS_PER_BLOCK * config::G_BLOCKVOLUME + iq::BLOCK_SIZE - 1) / iq::BLOCK_SIZE>(threadIdx.x, 3 * iq::NUM_COLUMNS_PER_BLOCK * config::G_BLOCKVOLUME); local_cell_index++){
			...
			iq::store_data_neigbours_coupling_solid(particle_buffer_solid, current_coupling_solid, W_velocity_solid, W1_pressure_interface, contact_area, normal[alpha]);
...
__global__ void simple_surface_flow_create_iq_system(const uint32_t num_blocks, Duration dt, const ParticleBuffer<MaterialTypeSolid> particle_buffer_solid, const ParticleBuffer<MaterialTypeFluid> particle_buffer_fluid, const ParticleBuffer<MaterialTypeSolid> next_particle_buffer_solid, const ParticleBuffer<MaterialTypeFluid> next_particle_buffer_fluid, const Partition prev_partition, const Partition partition, const Grid grid_solid, const Grid grid_fluid, const SurfaceParticleBuffer surface_particle_buffer_solid, const SurfaceParticleBuffer surface_particle_buffer_fluid, const SurfaceFlowParticleBuffer surface_flow_particle_buffer, SimpleSurfaceFlowIQCreatePointers iq_pointers) {
	...
	float coupling_solid_local[(3 * iq::NUM_COLUMNS_PER_BLOCK * config::G_BLOCKVOLUME + iq::BLOCK_SIZE - 1) / iq::BLOCK_SIZE];
	...
	*/
	for(size_t local_cell_index = 0; local_cell_index < iq::get_thread_count<iq::BLOCK_SIZE, (1 * config::G_BLOCKVOLUME + iq::BLOCK_SIZE - 1) / iq::BLOCK_SIZE>(threadIdx.x, config::G_BLOCKVOLUME); local_cell_index++){
		...
		if(pressure_domain_denominator_local[local_cell_index] > 0.0f || pressure_surface_denominator_local[local_cell_index] > 0.0f){
			pressure_fluid = (pressure_domain_nominator_local[local_cell_index] + pressure_surface_nominator_local[local_cell_index]) / (pressure_domain_denominator_local[local_cell_index] + pressure_surface_denominator_local[local_cell_index]);
		}else{
			pressure_fluid = 0.0f;
		}
		...
		const int row_index_pressure_domain = 2 * iq::NUM_ROWS_PER_BLOCK * num_blocks + base_row + row;
		const int row_index_pressure_surface = 3 * iq::NUM_ROWS_PER_BLOCK * num_blocks + base_row + row;
		...
		atomicAdd(&(iq_pointers.scaling_fluid[row_index_scaling]), (scaling_domain_local[local_cell_index] + scaling_surface_local[local_cell_index]) / dt.count());
	...
	for(size_t local_cell_index = 0; local_cell_index < iq::get_thread_count<iq::BLOCK_SIZE, (3 * config::G_BLOCKVOLUME + iq::BLOCK_SIZE - 1) / iq::BLOCK_SIZE>(threadIdx.x, 3 * config::G_BLOCKVOLUME); local_cell_index++){
		...
		if(mass_domain_local[local_cell_index] > 0.0f || mass_surface_local[local_cell_index] > 0.0f){
			mass_fluid = dt.count() / (mass_domain_local[local_cell_index] + mass_surface_local[local_cell_index]);
		}else{
			mass_fluid = 0.0f;
		}
		...
		const int row_index_velocity_domain = 6 * iq::NUM_ROWS_PER_BLOCK * num_blocks + 3 * (base_row + row) + alpha;
		const int row_index_velocity_surface = 9 * iq::NUM_ROWS_PER_BLOCK * num_blocks + 3 * (base_row + row) + alpha;
		
		atomicAdd(&(iq_pointers.iq_solve_velocity_result[row_index_velocity_solid]), velocity_solid_local[local_cell_index]);
		atomicAdd(&(iq_pointers.iq_solve_velocity_result[row_index_velocity_fluid]), (velocity_domain_local[local_cell_index] + velocity_surface_local[local_cell_index]));
	...
	for(size_t local_cell_index = 0; local_cell_index < iq::get_thread_count<iq::BLOCK_SIZE, (3 * iq::NUM_COLUMNS_PER_BLOCK * config::G_BLOCKVOLUME + iq::BLOCK_SIZE - 1) / iq::BLOCK_SIZE>(threadIdx.x, 3 * iq::NUM_COLUMNS_PER_BLOCK * config::G_BLOCKVOLUME); local_cell_index++){
		...
		atomicAdd(&(iq_pointers.gradient_fluid_values[column_index]), (gradient_domain_local[local_cell_index] + gradient_surface_local[local_cell_index]));
