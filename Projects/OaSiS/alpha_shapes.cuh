...
__forceinline__ __device__ void alpha_shapes_fetch_particles(const ParticleBuffer<MaterialType> particle_buffer, int* particle_indices, const int particles_in_cell, const int src_blockno, const ivec3 blockid_offset, const int cellno, const int start_index){
...
__forceinline__ __device__ void alpha_shapes_check_contact_condition(const ParticleBuffer<MaterialType> particle_buffer, const Partition prev_partition, AlphaShapesParticleBuffer alpha_shapes_particle_buffer, const int* own_particle_indices, std::array<int, 3>* triangles, bool* triangles_is_alpha, int* temporary_hull_triangles, int& current_triangle_count, int& next_triangle_count, int& temporary_hull_triangles_count, int& next_temporary_hull_triangles_count, const ivec3 blockid, const int triangle_index, const int p3_id, const bool is_alpha, const int finalize_particles_start, const int finalize_particles_end){
	...
	for(int contact_triangle_index = face_contacts - 1; contact_triangle_index >= 0; contact_triangle_index--) {//NOTE: Loop goes backwards to handle big indices first which allows easier swapping
		...
		if(triangles_is_alpha[contact_indices[contact_triangle_index]] != is_alpha){
			alpha_shapes_finalize_triangle(particle_buffer, prev_partition, alpha_shapes_particle_buffer, own_particle_indices, triangles, current_triangle_count, next_triangle_count, triangles[contact_indices[contact_triangle_index]], blockid, finalize_particles_start, finalize_particles_end);
		}
...
__forceinline__ __device__ void alpha_shapes_finalize_triangle(const ParticleBuffer<MaterialType> particle_buffer, const Partition prev_partition, AlphaShapesParticleBuffer alpha_shapes_particle_buffer, const int* particle_indices, const std::array<int, 3>* triangles, const int current_triangle_count, const  int next_triangle_count, const std::array<int, 3> triangle, const ivec3 blockid, const int range_start, const int range_end){
	...
	for(size_t contact_index = 0; contact_index < 3; ++contact_index){
		...
		for(int particle_id = range_start; particle_id < range_end; particle_id++) {
			if(triangle[contact_index] == particle_indices[particle_id]){
				found = true;
				break;
			}
		}
	...
	for(int particle_id = range_start; particle_id < range_end; particle_id++) {
		...
		int advection_source_blockno;
		int source_pidib;
		alpha_shapes_fetch_id(prev_partition, particle_indices[particle_id], blockid, advection_source_blockno, source_pidib);
		...
		for(int triangle_id = 0; triangle_id < current_triangle_count + next_triangle_count; ++triangle_id){
		...
		if(!triangle_vertex && in_convex_hull && in_halfspace_without_threshold){
...
__forceinline__ __device__ void alpha_shapes_build_tetrahedra(const ParticleBuffer<MaterialType> particle_buffer, const Partition prev_partition, AlphaShapesParticleBuffer alpha_shapes_particle_buffer, const int* particle_indices, const int particle_indices_count, const int* own_particle_indices, std::array<int, 3>* triangles, bool* triangles_is_alpha, int& current_triangle_count, int& next_triangle_count, const ivec3 blockid, const float alpha, const int finalize_particles_start, const int finalize_particles_end, const int* contact_triangles, int* additional_contact_particles, const int contact_triangles_count, const int additional_contact_particles_count, float& minimum_x, float& maximum_x){
	...
	int new_particle_count = additional_contact_particles_count;
	while(new_particle_count > 0){
		...
		int current_smallest_index = 0;//Default is 0
		for(int m = 0; m < new_particle_count - 1; ++m){
			const std::array<float, 3> current_outest_point_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, additional_contact_particles[m], blockid);
			...
			//Sort temporary_convex_hull such that all triangles in halfspace of current point are at front
			int in_halfspace_count = 0;
			for(int j = 0; j < temporary_convex_hull_triangles_count; ++j){
				const std::array<float, 3> current_p0_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, triangles[temporary_convex_hull[j]][0], blockid);
				const std::array<float, 3> current_p1_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, triangles[temporary_convex_hull[j]][1], blockid);
				const std::array<float, 3> current_p2_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, triangles[temporary_convex_hull[j]][2], blockid);
				...
				//Perform halfspace test
				const bool current_in_halfspace = current_triangle_normal_vec.dot(current_outest_point_position - current_p0_position) > ALPHA_SHAPES_HALFSPACE_TEST_THRESHOLD;
				
				//If point is in an outer halfspace we swap it to the front
				if(current_in_halfspace){
					thrust::swap(temporary_convex_hull[j], temporary_convex_hull[in_halfspace_count]);
					in_halfspace_count++;
				}
			}
			...
			for(int j = 0; j < new_particle_count - 1; ++j){
				const std::array<float, 3> current_particle_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, additional_contact_particles[j], blockid);
				...
				if(m != j){
					...
					for(int i = 0; i < in_halfspace_count; ++i){
						const std::array<float, 3> current_p0_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, triangles[temporary_convex_hull[i]][0], blockid);
						const std::array<float, 3> current_p1_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, triangles[temporary_convex_hull[i]][1], blockid);
						const std::array<float, 3> current_p2_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, triangles[temporary_convex_hull[i]][2], blockid);
						...
						for(int k = 0; k < in_halfspace_count; ++k){
							std::array<bool, 3> vertex_contact = {};
							if(
								(triangles[temporary_convex_hull[i]][0] == triangles[temporary_convex_hull[k]][0] || triangles[temporary_convex_hull[i]][0] == triangles[temporary_convex_hull[k]][1] || triangles[temporary_convex_hull[i]][0] == triangles[temporary_convex_hull[k]][2])
							){
								vertex_contact[0] = true;
							}
							if(
								(triangles[temporary_convex_hull[i]][1] == triangles[temporary_convex_hull[k]][0] || triangles[temporary_convex_hull[i]][1] == triangles[temporary_convex_hull[k]][1] || triangles[temporary_convex_hull[i]][1] == triangles[temporary_convex_hull[k]][2])
							){
								vertex_contact[1] = true;
							}
							if(
								(triangles[temporary_convex_hull[i]][2] == triangles[temporary_convex_hull[k]][0] || triangles[temporary_convex_hull[i]][2] == triangles[temporary_convex_hull[k]][1] || triangles[temporary_convex_hull[i]][2] == triangles[temporary_convex_hull[k]][2])
							){
		...
		const std::array<float, 3> particle_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, additional_contact_particles[current_smallest_index], blockid);
		const vec3 particle_position {particle_position_arr[0], particle_position_arr[1], particle_position_arr[2]};
		...
		for(int j = 0; j < temporary_convex_hull_triangles_count; ++j){
			...
			if((new_particle_count == (new_particle_count - 1)) || in_halfspace){
				vec3 sphere_center;
				float radius;
				...
				if(convex){
					...
					if(triangles_is_alpha[temporary_convex_hull[0]]){
						alpha_shapes_finalize_triangle(particle_buffer, prev_partition, alpha_shapes_particle_buffer, own_particle_indices, triangles, current_triangle_count, next_triangle_count, triangles[temporary_convex_hull[j]], blockid, finalize_particles_start, finalize_particles_end);
					}
		...
		thrust::swap(additional_contact_particles[current_smallest_index], additional_contact_particles[new_particle_count - 1]);
		new_particle_count--;
...
__forceinline__ __device__ void alpha_shapes_handle_triangle(const ParticleBuffer<MaterialType> particle_buffer, const Partition prev_partition, AlphaShapesParticleBuffer alpha_shapes_particle_buffer, const int* particle_indices, const int particle_indices_count, const int* own_particle_indices, std::array<int, 3>* triangles, bool* triangles_is_alpha, int& current_triangle_count, int& next_triangle_count, const ivec3 blockid, const float alpha, const int finalize_particles_start, const int finalize_particles_end, bool is_first_triangle, const int triangle_index, float& minimum_x, float& maximum_x){
	//TODO: Correct triangle count (can actually be max_triangle_count_for_used_points - num_used_points + 1, so find trick to avoid storing it)
	constexpr size_t ALPHA_SHAPES_MAX_CIRCUMSPHERE_TRIANGLES = ALPHA_SHAPES_MAX_CIRCUMSPHERE_POINTS / 3;
	...
	if(circumsphere_points_count > 0){
		...
		const std::array<float, 3> current_outest_point_position_arr = alpha_shapes_get_particle_position(particle_buffer, prev_partition, circumsphere_points[0], blockid);
		const vec3 current_outest_point_position {current_outest_point_position_arr[0], current_outest_point_position_arr[1], current_outest_point_position_arr[2]};
		...
		for(int i = 0; i < particle_indices_count; ++i){
			...
			if(on_correct_side){
				...
				for(int j = 0; j < circumsphere_triangles_count; ++j){
					...
					//Find outer edge (the one not being shared by another triangle in this set)
					std::array<int, 3> edge_count = {};//Count for opposite edge
	...
	thrust::swap(circumsphere_points[0], circumsphere_points[circumsphere_points_count - 1]);
	...
	if(circumsphere_points_count > 0){
		...
		if(is_first_triangle){
			...
			triangles_is_alpha[current_triangle_count + next_triangle_count] = first_is_alpha;
			next_triangle_count++;
		}
	}else{
		//Remove the face and move it to alpha if it is alpha
		...
		if(triangles_is_alpha[triangle_index]){
			alpha_shapes_finalize_triangle(particle_buffer, prev_partition, alpha_shapes_particle_buffer, own_particle_indices, triangles, current_triangle_count, next_triangle_count, triangles[triangle_index], blockid, finalize_particles_start, finalize_particles_end);
		}
...
__global__ void alpha_shapes(const ParticleBuffer<MaterialType> particle_buffer, const Partition prev_partition, const Partition partition, const Grid grid, AlphaShapesParticleBuffer alpha_shapes_particle_buffer, const unsigned int start_index) {
	...
	//FIXME: Currently only running with first thread
	if(threadIdx.x != 0){
		return;
	}
	...
	*/
	...
	for(int i = -static_cast<int>(ALPHA_SHAPES_KERNEL_SIZE); i <= static_cast<int>(ALPHA_SHAPES_KERNEL_SIZE); ++i){
		for(int j = -static_cast<int>(ALPHA_SHAPES_KERNEL_SIZE); j <= static_cast<int>(ALPHA_SHAPES_KERNEL_SIZE); ++j){
			for(int k = -static_cast<int>(ALPHA_SHAPES_KERNEL_SIZE); k <= static_cast<int>(ALPHA_SHAPES_KERNEL_SIZE); ++k){
				...
				alpha_shapes_fetch_particles(particle_buffer, particle_indices.data(), current_bucket_size, current_blockno, blockid_offset, current_cellno, particle_bucket_size);
	...
	for(int current_triangle_index = current_triangle_count - 1; current_triangle_index >=0; current_triangle_index--) {
		if(triangles_is_alpha[current_triangle_index]){
			alpha_shapes_finalize_triangle(particle_buffer, prev_partition, alpha_shapes_particle_buffer, own_particle_indices.data(), triangles.data(), current_triangle_count, next_triangle_count, triangles[current_triangle_index], blockid, own_last_active_particles_start, own_particle_bucket_size);
		}
	}
	
	//Finalize all particles left
	alpha_shapes_finalize_particles(particle_buffer, prev_partition, alpha_shapes_particle_buffer, own_particle_indices.data(), blockid, own_last_active_particles_start, own_particle_bucket_size);
}
