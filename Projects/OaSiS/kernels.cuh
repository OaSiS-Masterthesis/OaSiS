...
__global__ void copy_triangle_data_to_device(TriangleMesh triangle_mesh, uint32_t vertex_count, float* positions){
	const uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	if(idx >= vertex_count) {
		return;
	}
...
__global__ void copy_triangle_data_to_host(TriangleMesh triangle_mesh, uint32_t vertex_count, float* positions){
	const uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	if(idx >= vertex_count) {
		return;
	}
